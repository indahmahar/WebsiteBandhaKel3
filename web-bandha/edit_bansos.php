...
<html lang="en">
<head>
  <title>Halaman Edit Dana Bansos Bandha</title>
  <meta charset="utf-8">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-warning">
	  ...
	  <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            ...
            <li><a class="dropdown-item" href="chartbar.php">Grafik Dana</a></li>
                  ...
                  <a class="nav-link active"  href="edit_bansos.php">Edit Data Bansos</a>
...
<?php
		...
		if(isset($_GET['id'])){
			...
			if(mysqli_num_rows($tampil) == 0){
				echo '<div class="alert alert-warning">ID Tidak Ada </div>';
				exit();
			}else{
		...
		if ($error_no_kk =="" && $error_nik=="" && $error_nama_lengkap=="" && $error_jenis_kelamin=="" && $error_agama=="" && $error_tempat_lahir =="" && $error_tanggal_lahir == "" && $error_usia == "" && $error_status_pernikahan =="" && $error_jumlah_anggota_keluarga== "" && $error_alamat== "" && $error_rt== "" && $error_rw== "" && $error_pekerjaan== "" && $error_penghasilan== "" && $error_nomor_hp== "" && $error_ikut_jps== "" && $error_kriteria== "" && $error_keterangan== ""){
            ...
            $ikut_jps = $_POST['ikut_jps'];
            $kriteria = $_POST['kriteria'];
            $keterangan = $_POST['keterangan'];
			
            // query untuk update data
			$sql = mysqli_query($koneksi, "UPDATE bantuan_sosial SET nik='$nik', nama_lengkap='$nama_lengkap', jenis_kelamin='$jenis_kelamin', agama='$agama',  tempat_lahir='$tempat_lahir', tanggal_lahir='$tanggal_lahir', usia='$usia',  status_pernikahan='$status_pernikahan', jumlah_anggota_keluarga='$jumlah_anggota_keluarga', alamat='$alamat', rt='$rt', rw='$rw', pekerjaan='$pekerjaan', penghasilan='$penghasilan', nomor_hp='$nomor_hp', ikut_jps='$ikut_jps', kriteria='$kriteria', keterangan='$keterangan' WHERE id_bansos='$id'") or die(mysqli_error($koneksi));
			
            // pengecekan kondisi bisa diubah atau tidak
			if($sql){
				echo '<script>alert("Data Berhasil Di Ubah"); document.location="view.php?id='.$id.'";</script>';
			}else{
				echo '<div class="alert alert-warning">Pengubahan Data Tidak Berhasil, Silhkan Coba Kembali</div>';
			}
...
<div class="card-header"> DATA PRIBADI </div>
    ...
    <form method="post"  action="edit_bansos.php?id=<?php echo $id; ?>">
